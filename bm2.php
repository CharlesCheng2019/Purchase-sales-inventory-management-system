...
<?
//接收来自上一个页面的5个变量的值
$bmname=trim($_POST["bmname"]);
$bmaddress=trim($_POST["bmaddress"]);
...
$sql = "select * from bm where bmname='$bmname' ";
$result= mysqli_query($conn,$sql) OR die("<br/>ERROR: <b>".mysqli_error($conn)."</b><br/>产生问题的SQL：".$sql);
$row = mysqli_fetch_array($result);
if($row==0){//等于0表示不存在相同的用户名
$sql = "insert into bm(bmname,bmaddress,phone,fzr) values('$bmname','$bmaddress','$phone','$fzr')";

$result= mysqli_query($conn,$sql) OR die("<br/>ERROR: <b>".mysqli_error($conn)."</b><br/>产生问题的SQL：".$sql);
echo '录入成功，用户名为'.$bmname.'电话为'.$phone;
